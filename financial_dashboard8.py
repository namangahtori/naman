...
import json
from pypfopt import EfficientFrontier, risk_models, expected_returns  # Added missing import
...
if 'initialized' not in st.session_state:
    st.session_state.holdings = [
        {"ticker_or_name": "RELIANCE.NS", "amount": 50000, "asset_class": "Stocks", "expected_return": 0.1},
        {"ticker_or_name": "HDFCBANK.NS", "amount": 30000, "asset_class": "Stocks", "expected_return": 0.08}
    ]  # Added default holdings for testing
...
API_CONFIG = {
    "FMP_API_KEY": "your_actual_fmp_api_key",  # Replace with your actual key
    "ALPHA_VANTAGE_API_KEY": "your_actual_alpha_vantage_api_key"  # Replace with your actual key
}
...
def calculate_portfolio_risk_return(holdings):
    ...
    try:
        # Get historical data
        print("Debug: Fetching historical data for tickers:", tickers)
        data = get_historical_data(tickers)
        print("Debug: Historical data fetched:", data.shape if not data.empty else "Empty")
        if data.empty:
            raise ValueError("No historical data available")
            
        # Calculate returns and covariance
        returns = calculate_expected_returns(data)
        ...
        risk_free_rate = 0.05
        sharpe_ratio = (portfolio_ret - risk_free_rate) / portfolio_vol if portfolio_vol != 0 else 0
        
        return portfolio_ret, portfolio_vol, sharpe_ratio
    except Exception as e:
        print(f"Debug: Error in calculate_portfolio_risk_return: {e}")
...
page = st.sidebar.radio("", ["🏠 Dashboard Overview"])  # Temporarily limit to one page
...
if page == "🏠 Dashboard Overview":
    print("Debug: Starting Dashboard Overview")
    st.title("Financial Dashboard Overview")
    st.write("Debug: Dashboard Overview page loaded")
    
    # Calculate key metrics
    if st.session_state.holdings:
        print("Debug: Holdings:", st.session_state.holdings)
        ...
        with col3:
            ...
            try:
                ...
                print(f"Debug: Portfolio metrics - Return: {returns}, Risk: {risk}, Sharpe: {sharpe}")
            except Exception as e:
                st.error(f"Error calculating portfolio metrics: {e}")
                returns, risk, sharpe = 0, 0, 0
        ...
        with col1:
            ...
            if not holdings_df.empty:
                ...
                st.write("No holdings data available for asset allocation.")
